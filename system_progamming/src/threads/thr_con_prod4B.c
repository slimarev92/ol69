...
*                                                      *
*         SYSTEM PROGRAMMING: PRODUCER CONSUMER 4B     *
...
static pthread_mutex_t g_q_lock_p = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t g_q_lock_c = PTHREAD_MUTEX_INITIALIZER;
...
static void *Producer(void *args)
{
    ...
    while(ALWAYS)
    {
        ...
        pthread_mutex_unlock(&g_q_lock_p);
    }
...
static void *Consumer(void *args)
{
    ...
    while(ALWAYS)
    {
        ...
        /* used for easier viewing of printed values */
        sleep(1);

        pthread_mutex_lock(&g_q_lock_c);

        curr_num = g_task_q[g_curr_consume];
        g_curr_consume = (g_curr_consume + 1) % Q_SIZE;
        sem_post(&g_n_producible);

        pthread_mutex_unlock(&g_q_lock_c);
...
static int Run()
{
    ...
    pthread_mutex_destroy(&g_q_lock_p);
    pthread_mutex_destroy(&g_q_lock_c);
