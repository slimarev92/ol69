...
void CopyEnv(char **envp, char **envp_dest)
{
    ...
    while(NULL != *envp_runner) /* run on all env vars */
    {
        ...
        int curr_var_len = strlen(curr_var) + 1;        

        char *curr_var_dest = (char *) malloc(sizeof(char) * curr_var_len);
        ...
        for(i = 0; i < curr_var_len; ++i)
        {
            *(curr_var_dest + i) = tolower(*(curr_var + i));
        }
...
void SoldierArrayInit(int soldiers[], int num_of_soldiers)
{
    ...
    for(curr_soldier = 0; curr_soldier < (num_of_soldiers - 1);
             ++curr_soldier)
    {

        if(curr_soldier == num_of_soldiers)
        {
            soldiers[curr_soldier] = 0;
        }
        else
        {
            soldiers[curr_soldier] = curr_soldier + 1;
        }
    }
}

/* get next living soldier - O(N) */
...
int SoldierFind_N(int soldiers[], int num_of_soldiers)
{
    ...
    {
        int num_of_kills = num_of_soldiers / 2;

        num_of_kills += (num_of_soldiers % 2) == 0 ? 0 : 1;
